...
func anim_popup_score():
	get_node("pin_score_flag/pin_score_text").set_text(str(globals.score_pins))

func _on_pin_body_enter( body ):
	var touchbody = body.get_name()
	#if body.get_name() == "ball":
	print(touchbody + " touched pin")
	globals.score_pins += 1
	globals.score_multiplier += 1
	globals.score_total += globals.score_pins * globals.score_multiplier
